...
const BAY3_ASSIGNEES = [
  ...
  { displayName: "SILVANO SERTORIO HERNANDEZ", username: "employee13JZ", userId: "1932554285585764354" },
  { displayName: "LUIS VELAZQUEZ (13GN)", username: "lvelazquez", userId: "965" },
];
...
const ASSIGNEE_RULES: AssigneeRule[] = [
  { patterns: ["gurunanda", "guru nanda"], assignees: [BAY3_ASSIGNEES[0], BAY3_ASSIGNEES[1], BAY3_ASSIGNEES[2]] },
  { patterns: ["cmpc usa", "cmpc rolls", "cmpc usa (cut paper and rolls)"], assignees: [BAY3_ASSIGNEES[0], BAY3_ASSIGNEES[3]] },
  { patterns: ["north star"], assignees: [BAY3_ASSIGNEES[0], BAY3_ASSIGNEES[3]] },
  { patterns: ["karaka"], assignees: [BAY3_ASSIGNEES[0], BAY3_ASSIGNEES[2]] },
  { patterns: ["lennox"], assignees: [BAY3_ASSIGNEES[4], BAY3_ASSIGNEES[5]] },
  { patterns: ["tcl"], assignees: [BAY3_ASSIGNEES[6]] },
  { patterns: ["nzxt"], assignees: [BAY3_ASSIGNEES[6], BAY3_ASSIGNEES[7]] },
  { patterns: ["esi"], assignees: [BAY3_ASSIGNEES[7]] },
];
...
function resolveAssigneeFromHistory(historyMap: Map<string, string>, customerId?: string, customerName?: string, stableId?: string): Assignee | null {
  if (customerId && historyMap.has(customerId)) {
    ...
    const found = BAY3_ASSIGNEES.find((a) => a.userId === userId);
...
const ASSIGNED_STORAGE_KEY = `bay3_assigned_${TENANT_ID}_${FACILITY_ID}`;
const ASSIGNED_TODAY_KEY = `bay3_assigned_today_${TENANT_ID}_${FACILITY_ID}`;
    ...
    try {
      ...
      try {
        ...
        for (const lt of loadTasks) {
          ...
          const bay3Assignee = BAY3_ASSIGNEES.find((a) => a.userId === assigneeUid);
          ...
          shippingRows.push({
            ...
            location: dockId,
            entryId,
            assignee: bay3Assignee?.displayName || resolved?.displayName || assigneeName || "Unassigned",
            assigneeUserId: bay3Assignee?.userId || resolved?.userId || assigneeUid || "",
